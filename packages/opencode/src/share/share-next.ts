...
export namespace ShareNext {
  ...
  type QueueEntry = { timeout: NodeJS.Timeout; data: Map<string, Data> }
    ...
    state().then((s) => {
      s.unsubscribes.push(
        Bus.subscribe(Session.Event.Updated, async (evt) => {
          await s.sync(evt.properties.info.id, [
            {
              type: "session",
        ...
        Bus.subscribe(MessageV2.Event.Updated, async (evt) => {
          await s.sync(evt.properties.info.sessionID, [
            {
              type: "message",
              data: evt.properties.info,
            },
        ...
        Bus.subscribe(MessageV2.Event.PartUpdated, async (evt) => {
          await s.sync(evt.properties.part.sessionID, [
            {
              type: "part",
        ...
        Bus.subscribe(Session.Event.Diff, async (evt) => {
          await s.sync(evt.properties.sessionID, [
            {
              type: "session_diff",
